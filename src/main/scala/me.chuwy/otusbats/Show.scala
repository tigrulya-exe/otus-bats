...
object Show {
  ...
  implicit def intShow: Show[Int] = _.toString
  implicit def stringShow: Show[String] = identity
  implicit def booleanShow: Show[Boolean] = _.toString

  // 1.2 Instances with conditional implicit

  implicit def listShow[A](implicit ev: Show[A]): Show[List[A]] =
    (a: List[A]) => mkString_(a, "[", "]", ",")
